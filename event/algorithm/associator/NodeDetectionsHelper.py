...
class NodeDetectionsHelper:  # Or place in an appropriate existing class/module
    def has_existing_tracks_in_detection_space(
        self,
        new_detection_lla,
        existing_tracks_map,
        gating_threshold_m,
    ...
    def _get_node_rx_ecef(self, node_config_dict):
        ...
        if (
            not node_config_dict
            or "location" not in node_config_dict
            or "rx" not in node_config_dict["location"]
        ):
            return None
    ...
    def get_nodes_with_overlapping_detection_space(
        self,
        current_node_id_key,
        all_nodes_data,
        ...
        current_node_rx_ecef = self._get_node_rx_ecef(
            current_node_full_config["config"],
        )
        ...
        for other_node_id_key, other_node_full_data in all_nodes_data.items():
            ...
            distance_between_rxs = np.linalg.norm(
                current_node_rx_ecef - other_node_rx_ecef,
            )
