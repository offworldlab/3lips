    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if aircraft_data and aircraft_data.get('aircraft', []):
                ...
                print(f"   ✅ Synthetic-ADSB running with {len(aircraft_data.get('aircraft', []))} aircraft")
            else:
...
try:
    ...
    print(f'RETINASolver error: {e}')
        ...
        if 'RETINASolver available' in result.stdout:
            ...
            print(f"   ❌ RETINASolver integration failed: {result.stdout} {result.stderr}")
    except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if retina_logs:
                        print(f"   🔧 Recent RETINASolver activity:")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"   ⚠️  Synthetic-ADSB not responding")
            except Exception as e:
...
'''
    ...
    print("📝 Puppeteer validation script created: puppeteer_retina_solver_validation.js")
...
def run_integration_test():
    ...
    print(f"✅ Environment Setup: All components operational")
    print(f"📊 Track Monitoring: {len(monitoring_results['tracks_detected'])} tracks detected")
    print(f"🔧 RETINASolver Activity: {monitoring_results['retina_solver_calls']} calls")
    print(f"🎯 Successful Localizations: {monitoring_results['successful_localizations']}")
