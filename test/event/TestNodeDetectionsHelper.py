...
class TestNodeDetectionsHelper(unittest.TestCase):
    ...
    def test_has_existing_tracks_in_detection_space_true(self, mock_lla2ecef):
        ...
        result = self.helper.has_existing_tracks_in_detection_space(new_detection_lla, existing_tracks_map, gating_threshold_m)
    ...
    def test_has_existing_tracks_in_detection_space_false(self, mock_lla2ecef):
        ...
        new_detection_lla = [1, 2, 3]
        existing_tracks_map = {
            'track1': MockTrack([1000, 2000, 3000, 0, 0, 0])
        }
    ...
    def test__get_node_rx_ecef_valid(self, mock_lla2ecef):
        ...
        node_config = {
            'location': {
                'rx': {'latitude': 10, 'longitude': 20, 'altitude': 30}
            }
        }
    ...
    def test_get_nodes_with_overlapping_detection_space(self, mock_lla2ecef):
        ...
        all_nodes_data = {
            'node1': {'config': {'location': {'rx': {'latitude': 0, 'longitude': 0, 'altitude': 0}}}},
            'node2': {'config': {'location': {'rx': {'latitude': 1, 'longitude': 1, 'altitude': 1}}}},
            'node3': {'config': {'location': {'rx': {'latitude': 100, 'longitude': 100, 'altitude': 100}}}},
        }
        max_effective_range_m = 2
        result = self.helper.get_nodes_with_overlapping_detection_space('node1', all_nodes_data, max_effective_range_m)
        self.assertIn('node2', result)
        self.assertNotIn('node3', result)
        self.assertNotIn('node1', result)

    @patch('event.algorithm.geometry.Geometry.Geometry.lla2ecef')
    def test_get_nodes_with_overlapping_detection_space_missing_config(self, mock_lla2ecef):
        all_nodes_data = {
            'node1': {'config': {'location': {'rx': {'latitude': 0, 'longitude': 0, 'altitude': 0}}}},
